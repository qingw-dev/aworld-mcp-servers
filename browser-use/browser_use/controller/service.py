	...
	def __init__(
		...
		async def extract_content(
			...
			for iframe in page.frames:
				if iframe.url != page.url and not iframe.url.startswith('data:') and not iframe.url.startswith('about:blank'):
					content += f'\n\nIFRAME {iframe.url}:\n'
			...
			window_size=128000*3
			stride=64000*3
			parts=1 if len(content)-window_size<=0 else (len(content)-window_size+stride-1)//stride+1
			start_idx=0
			cnt=0
			pre_res=""
			while start_idx < len(content):
				chunk = content[start_idx:start_idx+window_size]
				cnt+=1
				prompt = 'Your task is to extract the content of the page. You will be given a part of a page, previous extraction results and a goal, and you should extract all relevant information around this goal from the page. If the goal is vague, summarize the page. Respond in json format. \nExtraction goal: {goal} \nPrevious extraction results: {pre_res} \nPage of part {cnt}/{parts}: {page}'
				template = PromptTemplate(input_variables=['goal', 'pre_res', 'cnt', 'parts', 'page'], template=prompt)
				try:
					output = await page_extraction_llm.ainvoke(template.format(goal=goal, pre_res=pre_res, cnt=cnt, parts=parts, page=chunk))
					pre_res=output.content
					msg = f'📄  Extracted from page\n: {pre_res}\n'
					logger.info(msg)
					# return ActionResult(extracted_content=msg, include_in_memory=True)
				except Exception as e:
					...
					logger.info(chunk)
					# return ActionResult(extracted_content=msg)
				if start_idx+window_size>=len(content):
					break
				start_idx += stride
			return ActionResult(extracted_content=f'📄  Extracted from page\n: {pre_res}\n', include_in_memory=True)
		...
		async def scroll_down(params: ScrollAction, browser: BrowserContext):
			...
			if params.amount is not None:
				...
				await page.mouse.move(window_width//2, window_height//2)
				await page.mouse.wheel(0, window_height)
