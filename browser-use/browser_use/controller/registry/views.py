...
class RegisteredAction(BaseModel):
	...
	def prompt_description_old(self) -> str:
	...
	def prompt_description(self) -> str:
		...
		s = f'{self.description}: \n'
		s += '{' + str(self.name) + ': '
		json_schema=self.param_model.model_json_schema()
			...
			if isinstance(schema, dict):
				if '$ref' in schema:
					...
					if ref.startswith('#/$defs/'):
						...
						return resolve_refs(definitions.get(ref_name, {}).get("properties"), definitions)
						# return {ref_name: {**resolve_refs(definitions.get(ref_name, {}).get("properties"), definitions),**{"title":ref_name,"type":schema.get("type","object")}}}
		...
		resolved_properties = {k: resolve_refs(v, definitions) for k, v in properties.items()}
