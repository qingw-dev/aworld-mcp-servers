...
def log_performance(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            logger.error(f"[{request_id}] Failed {func.__name__} - Error: {str(e)} - Response time: {response_time:.3f}s")
            raise
...
def search_google(
    ...
    try:
        ...
        if "items" in data:
            ...
            logger.info(f"[{request_id}] Google search completed for '{query}' - Found {len(results)} results in {search_time:.3f}s")
        ...
        search_time = time.time() - search_start
        logger.error(f"[{request_id}] Unexpected error during Google search for '{query}': {e} - Time: {search_time:.3f}s")
        update_metrics('failed_searches')
        return [], False
...
def fetch_web_content(url: str, request_id: str = None) -> Tuple[str, bool]:
    ...
    try:
        ...
        logger.info(f"[{request_id}] Successfully fetched content from {url} - Length: {content_length} chars - Time: {fetch_time:.3f}s")
...
def process_single_query(
    ...
    search_results, search_success = search_google(
        query, api_key, cse_id, num=num_results,
        safe_search=safe_search, language=language, country=country,
    ...
    logger.info(f"[{request_id}] Completed processing query: {query} - Results: {len(query_output)} - Time: {query_time:.3f}s")
...
def process_queries_concurrent(
    ...
    with ThreadPoolExecutor(max_workers=min(MAX_WORKERS, len(queries))) as executor:
        future_to_query = {
            executor.submit(
                ...
                language, country, safe_search, request_id
    ...
    logger.info(f"[{request_id}] Completed concurrent processing - Total queries: {len(queries)}, Successful: {successful_queries}, Time: {process_time:.3f}s")
...
def health_check():
    ...
    content_success_rate = (current_metrics['successful_content_fetches'] / max(current_metrics['total_content_fetches'], 1)) * 100
...
def search_endpoint():
    ...
    try:
        ...
        logger.info(f"[{request_id}] Processing search request - Queries: {len(queries)}, Results per query: {num_results}")
        
        # Process queries concurrently
        results = process_queries_concurrent(
            queries, api_key, cse_id, num_results, 
            language, country, safe_search, request_id
        )
...
def single_search_endpoint():
    ...
    try:
        ...
        logger.info(f"[{request_id}] Processing single search request - Query: '{query}', Fetch content: {fetch_content}")
...
def batch_search_endpoint():
    ...
    try:
        ...
        logger.info(f"[{request_id}] Processing batch search request - Searches: {len(data['searches'])}, Fetch content: {fetch_content}")
        ...
        def process_batch_search(search_item, index):
            ...
            if fetch_content and search_results:
                ...
                with ThreadPoolExecutor(max_workers=min(MAX_CONTENT_WORKERS, len(search_results))) as content_executor:
                    content_futures = {content_executor.submit(fetch_batch_result_content, result): result for result in search_results}
        ...
        with ThreadPoolExecutor(max_workers=min(MAX_WORKERS, len(data["searches"]))) as executor:
            future_to_index = {executor.submit(process_batch_search, search_item, i): i for i, search_item in enumerate(data["searches"])}
        ...
        results.sort(key=lambda x: next(i for i, search in enumerate(data["searches"]) if search["query"] == x["query"]))
...
if __name__ == "__main__":
    ...
    logger.info(f"Configuration - Max workers: {MAX_WORKERS}, Max content workers: {MAX_CONTENT_WORKERS}, Request timeout: {REQUEST_TIMEOUT}s")
