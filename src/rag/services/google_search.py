...
class GoogleSearchService:
    ...
    def search(
        ...
        try:
            ...
            response = requests.get(self.service_url, params=params, timeout=self.settings.request_timeout)
            ...
            if "items" in data:
                for item in data["items"]:
                    results.append(SearchResult(url=item.get("link", ""), abstract=item.get("snippet", "")))
            ...
            search_time = time.time() - search_start
            self.logger.error(f"[{request_id}] JSON decode error for '{query}': {e} - Time: {search_time:.3f}s")
            self.metrics.increment("failed_searches")
            return [], False
